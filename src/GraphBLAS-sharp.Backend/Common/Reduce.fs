    ...
    let atomicRun<'a when 'a: struct>
        ...
        let reduce =
            <@ fun (ndRange: Range1D) length (inputArray: ClArray<'a>) (totalSum: ClCell<'a>) ->
                ...
                if lid = 0 then
                    atomic %opAdd totalSum.Value localValues.[0]
                    |> ignore @>
        ...
        fun (processor: MailboxProcessor<_>) (valuesArray: ClArray<'a>) ->
            ...
            let totalSum = clContext.CreateClCell(zero)

            let kernel = kernel.GetKernel()

            processor.Post(
                Msg.MsgSetArguments(fun () -> kernel.KernelFunc ndRange valuesArray.Length valuesArray totalSum)
            )
