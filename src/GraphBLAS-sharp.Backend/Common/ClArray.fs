...
module ClArray =
    ...
    let copy (clContext: ClContext) workGroupSize =
        ...
        fun (processor: MailboxProcessor<_>) (inputArray: ClArray<'a>) ->
            let outputArray = clContext.CreateClArray(inputArray.Length, allocationMode = AllocationMode.Default)
    ...
    let replicate (clContext: ClContext) =
        ...
        fun (processor: MailboxProcessor<_>) workGroupSize (inputArray: ClArray<'a>) count ->
            ...
            let outputArray = clContext.CreateClArray(outputArrayLength, allocationMode = AllocationMode.Default)
    ...
    let private update (clContext: ClContext) =
        let update =
            <@
                fun (ndRange: Range1D)
                    inputArrayLength
                    bunchLength
                    (resultBuffer: ClArray<int>)
                    (verticesBuffer: ClArray<int>) ->
        ...
        fun (processor: MailboxProcessor<_>) workGroupSize (inputArray: ClArray<int>) (inputArrayLength: int) (vertices: ClArray<int>) (bunchLength: int) ->
            ...
            processor.Post(
                Msg.MsgSetArguments(fun () ->
                    kernel.KernelFunc ndRange inputArrayLength bunchLength inputArray vertices
                )
            )
    ...
    let private scan (clContext: ClContext) workGroupSize =
        ...
        fun (processor: MailboxProcessor<_>) (inputArray: ClArray<int>) (inputArrayLength: int) (vertices: ClArray<int>) (verticesLength: int) (totalSum: ClCell<int>) ->
            ...
            processor.Post(
                Msg.MsgSetArguments(fun () ->
                    kernel.KernelFunc ndRange inputArrayLength verticesLength inputArray vertices totalSum
                )
            )
    ...
    let prefixSumExclude (clContext: ClContext) workGroupSize =
        ...
        let prefixSumExcludeInplace = prefixSumExcludeInplace clContext workGroupSize
    ...
    let prefixSumInclude (clContext: ClContext) workGroupSize =
        ...
        fun (processor: MailboxProcessor<_>) (inputArray: ClArray<int>) ->
            ...
            let _, totalSum = prefixSumExcludeInplace processor copiedArray totalSum

            let outputArray = clContext.CreateClArray(inputArrayLength, allocationMode = AllocationMode.Default)
            ...
            processor.Post(
                Msg.MsgSetArguments(fun () ->
                    kernel.KernelFunc ndRange copiedArray inputArrayLength totalSum outputArray
                )
            )
    ...
    let setPositions (clContext: ClContext) =
        ...
        fun (processor: MailboxProcessor<_>) workGroupSize (inputArray: ClArray<int>) (positions: ClArray<int>) (outputArraySize: int) ->
            let ndRange = Range1D.CreateValid(inputArray.Length, workGroupSize)

            let outputArray = clContext.CreateClArray(outputArraySize, allocationMode = AllocationMode.Default)

            let kernel = kernel.GetKernel()

            processor.Post(
                Msg.MsgSetArguments(fun () ->
                    kernel.KernelFunc ndRange inputArray inputArray.Length positions outputArray
                )
            )
    ...
    let removeDuplications (clContext: ClContext) workGroupSize =
        ...
        fun (processor: MailboxProcessor<_>) (inputArray: ClArray<int>) ->
            ...
            let resultLength =
                let a = [| 0 |]
                let _ = processor.PostAndReply(fun ch -> Msg.CreateToHostMsg(sum, a, ch))
                a.[0]

            let outputArray = setPositions processor workGroupSize inputArray positions resultLength
