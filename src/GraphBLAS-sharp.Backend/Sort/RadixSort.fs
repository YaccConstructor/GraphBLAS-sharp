...
module RadixSort =
    ...
    //         @>
    //     let program = clContext.Compile(updatePositions)
    ...
    let private createHistograms
        ...
        let createHistograms =
            ...
            @>
        let program = clContext.Compile(createHistograms)
        ...
        fun (processor: MailboxProcessor<_>)
            ...
            let sectorIndices, sectorsAmountGPU = scanInclude processor sectorHeads (clContext.CreateClArray(1)) 0

            // let sectorsAmount =
            //     ClCell.toHost sectorsAmountGPU
            //     |> ClTask.runSync clContext
            let sectorsAmount = processor.PostAndReply(fun ch -> Msg.CreateToHostMsg(sectorsAmountGPU, [|0|], ch)).[0]
            ...
            <@
                fun (range: Range1D)
                    ...
                    if workGroupNumber < workGroupAmount then
                        ...
                        barrierLocal()
            @>
        let program = clContext.Compile(permute)
    ...
    //                         bucketHeads.[idx] <- 0
    //         @>
    //     let program = clContext.Compile(computeGarbage)
    ...
    //                         keysAncilla.[idx] <- key
    //         @>
    //     let program = clContext.Compile(repair)
        ...
        fun (processor: MailboxProcessor<_>)
            ...
            for globalStep in (32 - log) / 2 .. 32 / 2 - 1 do
                ...
                while not isFinished do
                    ...
                    let isFinishedGPU = clContext.CreateClArray<bool> [|isFinished|]
                    ...
                    //     |> ClTask.runSync clContext
                    isFinished <- processor.PostAndReply(fun ch -> Msg.CreateToHostMsg(isFinishedGPU, [|true|], ch)).[0]
