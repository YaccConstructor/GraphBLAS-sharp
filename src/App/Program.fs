...
let main argv =
    ...
    let rmtx = CSRMatrix.spgemm clContext workGroupSize processor fmtx smtx times plus 0.0
    ...
    // let n = 100
    // let xs = clContext.CreateClArray(n)
    // let goo = clContext.CreateClCell()
    // let init =
    //     <@
    //         fun (range: Range1D)
    //             (gooBuff: ClCell<_>)
    ...
    //             let bib = gooBuff.Value
    //             gooBuff.Value <- bib
    //             if i < n then
    //                 buffer.[i] <- i
    //     @>
    // let kernel = clContext.CreateClKernel(init)
    // printfn "%A" kernel.Code
