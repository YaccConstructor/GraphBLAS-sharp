...
open Brahma.FSharp.ClTaskImpl
open Brahma.FSharp.ClTask
open Brahma.FSharp

type GraphblasContext = { ClContext: RuntimeContext }

type GraphblasEvaluation<'a> = EvalGB of (GraphblasContext -> 'a)

module EvalGB =
    let defaultEnv = { ClContext = RuntimeContext(ClDevice.GetFirstAppropriateDevice()) }
