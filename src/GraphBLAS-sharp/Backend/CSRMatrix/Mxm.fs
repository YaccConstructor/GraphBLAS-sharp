...
module internal rec Mxm =
    ...
    let private getNumberOfIntermediateProducts (leftMatrix: CSRMatrix<'a>) (rightMatrix: CSRMatrix<'a>) = opencl {
        ...
        do! RunCommand kernel <| fun kernelPrepare ->
            let range = _1D(Utils.workSize leftRowCount, Utils.workGroupSize)
            kernelPrepare
                range
    ...
    let getNnzEstimation
                ...
                <@
                    fun (ndRange: _1D)
                        ...
                        if binId < binOffset + binLength then
                            for j in leftPointers.[rowIdx] .. Pwarp .. leftPointers.[rowIdx + 1] - 1 do
        ...
        let symbolicWG binId nnzEstimation = opencl {
            ...
            do! RunCommand kernel <| fun kernelPrepare ->
                kernelPrepare
                <| _1D(binLength * wgSize |> Utils.workSize, wgSize)
                <| flatBins
                <| binsPointers.[binId]
                <| binLength
                <| leftMatrix.RowPointers
