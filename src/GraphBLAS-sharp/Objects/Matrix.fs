...
type Matrix<'a when 'a: struct> =
    ...
    static member FromBackend (q: MailboxProcessor<_>) matrix =
        ...
        | Backend.MatrixCOO m ->
            ...
            let _ = q.Post(Msg.CreateToHostMsg(m.Rows, rows))
            let _ = q.Post(Msg.CreateToHostMsg(m.Columns, columns))
            let _ = q.PostAndReply(fun ch -> Msg.CreateToHostMsg(m.Values, values, ch))
        ...
        | Backend.MatrixCSR m ->
            ...
            let _ = q.Post(Msg.CreateToHostMsg(m.RowPointers, rows))
            let _ = q.Post(Msg.CreateToHostMsg(m.Columns, columns))
            let _ = q.PostAndReply(fun ch -> Msg.CreateToHostMsg(m.Values, values, ch))
